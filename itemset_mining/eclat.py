...
class Eclat:
    ...
    def prune_items(self):
        ...
        for i in range(len(self.data)-1, -1, -1):
            if self.data[i].support < self.minsup:
                del self.data[i]
        self.data.sort()
        return
    ...
    def eclat_mine(self, data=[], prefix=[], closure=[], clen = 0, depth=0):
        ...
        if depth == 0:
            counter = 1
            if self.output:
                self.printset([], self.trans_count)
            if self.outfilename:
            ...
            for j in range(i+1, len(data)):
                ...
                if tmpset.support >= self.minsup:
                    if set1.support == tmpset.support:
                        if len(closure) <= cl:
                            closure.append(tmpset.suffix)
                        else:
                            closure[cl] = tmpset.suffix
                        cl += 1
                    else:
                        children.append(tmpset)
            
            if self.output:
                self.printset(prefix, set1.support, closure[:cl])
            if self.outfilename:
    ...
    def printclosed(prefx, supp, closure=[]):
    ...
    def saveclosed(prefx, supp, closure=[]):
        for item in prefix: self.out.write(str(item) + ' ')
    ...
    def run(self, maxdepth=0):
        ...
        self.read_data()
        if self.verbose:
            print self.item_count, 'items,', self.trans_count, 'transactions.'
        self.prune_items()
        if self.verbose:
        ...
        f_count = self.eclat_mine(self.data)
        if self.outfilename:
        	self.out.close()
        if self.verbose:
            print 'done.'
...
e = Eclat(filename, minsup, output=options.output, outfilename=options.outfile, maxdepth=options.maxsize, verbose=options.verbose)
e.run()
